"""
📊 NASDAQ Model Leaderboard & Interactive Backtester
────────────────────────────────────────────────────
...
📁 Inputs:
──────────
- `model_performance_summary.csv` — generated by evaluation pipeline
- Trained models (.pkl)
...
def custom_shift_column(df, col, shift_n, debug=DEBUG_MODE):
    """
    📚 Shifts a column forward or backward with NaN padding.
    📥 Parameters:
    df: pd.DataFrame — input DataFrame
    col: str — column name to shift
    shift_n: int — number of rows to shift (positive or negative)
    debug: bool — enable debug logging
    📤 Returns: pd.Series — shifted column aligned with original index
    """
...
def get_safe_window(df, center_idx, before=20, after=25):
    """
    📚 Returns a safe slice of rows around a given integer index,
    guaranteeing that df has integer-based indexing.
    📥 Parameters:
    df: pd.DataFrame — input DataFrame
    center_idx: int — center row index
    before: int — rows before center
    after: int — rows after center
    📤 Returns: pd.DataFrame — sliced window of rows
    """

    if not isinstance(center_idx, int):
        raise ValueError("center_idx must be an integer row position.")

    df = df.reset_index(drop=True)  # ⛑ ensures integer-based indexing
...
def get_future_price(df, col, idx, shift_n):
    """
    📚 Gets the future value of a column at a shifted index.
    📥 Parameters:
    df: pd.DataFrame — input DataFrame
    col: str — column to extract from
    idx: int — current row index
    shift_n: int — how far to look ahead or behind
    📤 Returns: float or value — future value at shifted index
    """
...
def safe_shift_diff(df, col, shift_n, debug=False):
    ...
    shift_n: int — shift amount
    debug: bool — enable debug logging
    📤 Returns: pd.Series — absolute difference (or timedelta if datetime)
    """
...
def run_backtest(stock_file, model_name, selected_time, num_shares=100, debug=DEBUG_MODE):
    ...
    📤 Returns:
    ──────────
    None — Outputs results directly to the Streamlit interface
    ...
    try:
        ...
        # 3️⃣ Prepare row for prediction
        ...
        # row = entry_row
        # 🧠 Scan forward for first buy signal after selected date
        ...
        # 🚨 Handle case: no signal found
        if entry_idx is None:
            st.warning("⚠️ No BUY signal detected after the selected date.")
            return
        ...
        if debug:
            ...
            st.write(f"🔍 DEBUG: Evaluating error on col {close_col}")
            st.write(f"Type: {test_df[close_col].dtype}, sample: {test_df[close_col].iloc[0]}")
            st.write(f"Index type: {type(test_df.index)}")

        if close_col not in candle_df.columns:
            st.warning(f"⚠️ Column `{close_col}` not found in candlestick DataFrame.")
            st.write("🔍 Available columns:", candle_df.columns.tolist())
            return
        ...
        unix_ts = ts_val.timestamp()  # 🔥 Proper numeric timestamp for add_vline
        ...
        # 🎨 Layout settings
        fig.update_layout(
            title="📉 Price Action & Volume with Signal Markers",
            xaxis_title="Date",
            yaxis_title="Price ($)",
            height=650,
...
def load_features(file_path):
    """
    📚 Loads a feature dataset from a .parquet file and ensures a Datetime column exists.
    📥 Parameters:
    file_path: str — path to the .parquet file
    📤 Returns: pd.DataFrame — cleaned feature dataset
    """
...
def extract_price_column(df, debug=DEBUG_MODE):
    """
    📚 Extracts the first column that starts with "Close_".
    📥 Parameters:
    df: pd.DataFrame — input DataFrame
    debug: bool — enable debug logging
    📤 Returns: str — name of the close price column
    """
    try:
...
def evaluate_model_on_df(model, df, close_col):
    """
    📚 Evaluates a model on a given DataFrame and returns performance metrics.
    📥 Parameters:
    model: sklearn-compatible model
    df: pd.DataFrame — feature dataset
    close_col: str — name of the close price column
    📤 Returns: [f1, precision, recall, hit_rate, avg_abs_err, avg_prob]
    """
...
def stock_list():
    """
    📚 Scrapes a list of NASDAQ stocks from StockAnalysis.com..
    📥 Parameters: None
    📤 Returns: List[str] — formatted as "Company Name - SYMBOL"
    """
...
def main():
    """
    📚 Evaluates all models and generates a performance summary CSV.
    📥 Parameters: None
    📤 Returns: None (outputs to Streamlit and saves CSV)
    ...
    for i, model_path in enumerate(model_files):
        ...
        status_text.text(f"🔄 Evaluating model {i+1}/{total_models}: `{model_name}`")
    ...
    progress_bar.empty()
    status_text.success("✅ Model evaluation complete!")
...
if __name__ == "__main__":
    ...
    # Run evaluation
    if st.button("🚀 Run Evaluation and Generate Report"):
    ...
    if os.path.exists(REPORT_NAME):
        ...
        # 🔢 Rank models by performance (Test_F1_avg desc)
        ...
        with st.sidebar:
            ...
            st.session_state.min_f1 = st.slider("🔍 Min Test F1", 0.0, 1.0, st.session_state.min_f1, 0.05, key="f1")
            st.session_state.max_err = st.slider("🧯 Max Avg Error", 0.0, 25.0, st.session_state.max_err, 0.5, key="err")
            st.session_state.min_hit = st.slider("🎯 Min Hit Rate", 0.0, 1.0, st.session_state.min_hit, 0.05, key="hit")
        ...
        with st.expander("📊 Model Performance Leaderboard — What does this show?"):
            st.markdown("""
            Each row is a model:
            - **F1, Hit Rate, Prob Avg** → _Higher is better_
            - **Avg Error** → _Lower is better_
        ...
        try:
            ...
            st.warning(f"⚠️ Table display failed: {e}")
