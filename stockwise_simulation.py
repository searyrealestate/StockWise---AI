...
from strategy_engine import StockWiseAI 
...
import shap
import yfinance as yf  # <--- Fixes: NameError: name 'yf' is not defined
import glob            # <--- Fixes: NameError: name 'glob' is not defined
import urllib.request  # <--- Proactive Fix: Required for S&P500 scraper

# --- GEN-12 SYSTEM IMPORTS (CORRECTED) ---
import system_config as cfg
from system_config import clean_raw_data  # Moved from utils
from data_source_manager import DataSourceManager
from feature_engine import RobustFeatureCalculator, TechnicalAnalyzer # Moved from technical_analyzer
from notification_manager import NotificationManager
from stock_hunter import run_full_market_scan # Fixed: Use stock_hunter, not scanner

# Consolidated Strategy Classes
from strategy_engine import (
    StrategyOrchestra, 
    MichaAdvisor,       # Moved from mico_system
    AI_ParamPredictor   # Moved from mico_ai_module
)

# Consolidated Portfolio Classes
from portfolio_manager import RiskManager # Moved from risk_manager

# --- MISSING MODULES STUB (Prevents Crash) ---
# These files are not in your Gen-12 list. We mock them to allow the UI to load.
results_analyzer = None
mico_optimizer = None
MeanReversionAdvisor = None
BreakoutAdvisor = None
SuperTrendAdvisor = None
MovingAverageCrossoverAdvisor = None
VolumeMomentumAdvisor = None
...
def setup_logging_from_st():
    ...
    cfg.LoggerSetup.setup_logger("StockWise_UI", log_file="stockwise_ui.log", level=log_level)
    logger = logging.getLogger(__name__)
...
class ProfessionalStockAdvisor:
    def __init__(self, model_dir: str, data_source_manager=None, debug=False, testing_mode=False, download_log=False,
        ...
        self.models = {} # Initialize as empty to prevent crashes elsewhere
        self.feature_names = {}
    ...
    def run_analysis(self, full_stock_data, ticker_symbol, analysis_date, use_market_filter=True):
        """
        v13 Compliant Analysis: Delegates logic to StrategyOrchestra.
        """
        # --- 1. Initialize v13 Brain ---
        orchestra = StrategyOrchestra()
        self.log.info(f"--- v13 Analysis started for {ticker_symbol} on {analysis_date} ---")

        # --- 2. Data Preparation ---
        if full_stock_data is None or full_stock_data.empty:
            return pd.DataFrame(), {'action': "WAIT", 'reason': "No Data"}
        
        full_stock_data = clean_raw_data(full_stock_data)
        
        # Prevent Lookahead: Slice data strictly up to analysis_date
        analysis_dt = pd.to_datetime(analysis_date)
        data_up_to_date = full_stock_data[full_stock_data.index <= analysis_dt]
        
        if data_up_to_date.empty:
            return full_stock_data, {'action': "WAIT", 'reason': "No Data for Date"}

        # --- 3. Feature Engineering (The 63 Models) ---
        featured_data = self.calculator.calculate_all_features(data_up_to_date)
        if featured_data.empty:
            return full_stock_data, {'action': "WAIT", 'reason': "Feature Calc Failed"}
            
        latest_row = featured_data.iloc[-1]
        all_features_dict = latest_row.to_dict()
        price_on_date = latest_row['close']

        # --- 4. Get Context Data ---
        # A. Market Health (Macro)
        health_score, health_reasons = self.get_market_health_index(analysis_date)
        market_state = "BEAR" if health_score < 2 else "NEUTRAL"
        
        if use_market_filter and market_state == "BEAR" and health_score < 3:
             return full_stock_data, {
                 'action': "WAIT", 
                 'confidence': 100.0,
                 'reason': f"Market Filter: Score {health_score}/4 (Requires 3)",
                 'agent': "System"
             }

        # B. Fundamentals (Required for 'Strategic' Agent)
        fundamentals = self.data_source_manager.get_fundamentals(ticker_symbol)
        
        # C. AI Confidence (Gen-12 Core B)
        # We instantiate the AI Specialist for this specific symbol
        ai_conf = 0.5
        try:
            # Initialize the AI Brain for this specific ticker
            brain = StockWiseAI(symbol=ticker_symbol)
            
            # Prepare data window (Last 60 bars for the LSTM)
            if len(data_up_to_date) >= 60:
                df_window = data_up_to_date.tail(60)
                
                # Ask the brain for a prediction
                # features and fundamentals are passed for logging/traceability
                decision, prob, trace = brain.predict_trade_confidence(
                ...
                ai_conf = prob
                self.log.info(f"Gen-12 AI Confidence for {ticker_symbol}: {ai_conf:.2%}")
            else:
                self.log.warning(f"Not enough data for AI prediction (Need 60, got {len(data_up_to_date)})")

        except Exception as e:
            self.log.error(f"AI Prediction Engine Failed: {e}", exc_info=True)
            ai_conf = 0.0

        # --- 5. EXECUTE STRATEGY ORCHESTRA (The Fix) ---
        # This calls the OFFICIAL v13 Logic from strategy_engine.py
        
        decision = orchestra.decide_action(
        ...
        result = {
            ...
            'shap_values': None, # SHAP is computationally expensive, optional here
            'shap_base_value': 0
        }

        if decision:
            # Unpack Tuple: (Action, Prob, Price, Agent, SL, TP, Score)
            ...
            result.update({
                ...
                'reason': f"Strategy: {agent} | Score: {score}/100"
            })
        else:
            result.update({
                'action': "WAIT",
                ...
                'agent': "System",
                'reason': "No strategy threshold met (Score < 75)"
            })
...
def create_enhanced_interface(IS_CLOUD=False):
    ...
    with st.sidebar.expander("🛠️ Strategy & Model Config", expanded=False):
        ...
        # Gen-12 does not use static targets (1%, 2%). It uses Dynamic AI.
        AGENT_CONFIGS = {
            'Gen-12 Neural (Universal)': "models", 
            'Gen-12 Specialist (Stock-Specific)': "models" 
        }
    ...
    mode = st.sidebar.radio("Select Operation Mode",
                            ["🎯 Single Analysis", "🚀 Market Screener", "🗓️ System Backtest", "🧠 AI Training", "Visual Strategy Debugger"],
                            index=0
                            )
    ...
    # elif mode == "Visual Strategy Debugger":
    #     st.header("Visual Strategy Debugger (Gen-13)")
    #     st.markdown("Analyze raw indicators, Noise Reduction outputs, and Dynamic Stops directly on the chart.")

    #     col_db1, col_db2, col_db3 = st.columns(3)
    #     dbg_symbol = col_db1.text_input("Ticker to Debug", "NVDA", key="dbg_sym").upper()
    #     dbg_days = col_db2.number_input("Days Back", min_value=50, max_value=500, value=150)
    #     dbg_run = col_db3.button("Load Diagnostic Chart", use_container_width=True)

    #     st.sidebar.subheader("Chart Overlays & Indicators")
    #     show_sma = st.sidebar.checkbox("Show SMA (20, 50)", value=True)
    #     show_bb = st.sidebar.checkbox("Show Bollinger Bands", value=False)
    #     show_atr_stop = st.sidebar.checkbox("Gen-13 Dynamic Stop (ATR)", value=True)

    #     if dbg_run and dbg_symbol:
    #         with st.spinner(f"Fetching data and computing Gen-13 features for {dbg_symbol}..."):
    #             # Fetch raw data
    #             df_dbg = st.session_state.data_manager.get_stock_data(dbg_symbol, days_back=dbg_days)
                
    #             if df_dbg is not None and not df_dbg.empty:
    #                 # Apply Feature Engine
    #                 calc = st.session_state.advisor.calculator
    #                 try:
    #                     df_dbg = calc.calculate_features(df_dbg, context_data=st.session_state.contextual_data)
    #                 except Exception as e:
    #                     st.warning(f"Feature calculation ran with warnings: {e}")

    #                 # Build Diagnostic Plotly Chart
    #                 fig = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.7, 0.3], vertical_spacing=0.05)
                    
    #                 # Candlestick
    #                 fig.add_trace(go.Candlestick(
    #                     x=df_dbg.index, open=df_dbg['open'], high=df_dbg['high'], low=df_dbg['low'], close=df_dbg['close'], 
    #                     name="Price Action"
    #                 ), row=1, col=1)

    #                 # Technical Overlays
    #                 if show_sma and 'sma_20' in df_dbg.columns:
    #                     fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg['sma_20'], line=dict(color='cyan', width=1.5), name="SMA 20"), row=1, col=1)
    #                     fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg['sma_50'], line=dict(color='orange', width=1.5), name="SMA 50"), row=1, col=1)

    #                 if show_bb and 'bb_upper' in df_dbg.columns:
    #                     fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg['bb_upper'], line=dict(color='gray', width=1, dash='dot'), name="BB Upper"), row=1, col=1)
    #                     fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg['bb_lower'], line=dict(color='gray', width=1, dash='dot'), name="BB Lower"), row=1, col=1)

    #                 if show_atr_stop and 'atr' in df_dbg.columns:
    #                     # Visualize the exact ML trailing stop distance
    #                     df_dbg['stop_level'] = df_dbg['close'] - (df_dbg['atr'] * 1.5)
    #                     fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg['stop_level'], line=dict(color='red', width=1.5, dash='dash'), name="ATR Dynamic Stop"), row=1, col=1)

    #                 # Volume
    #                 fig.add_trace(go.Bar(x=df_dbg.index, y=df_dbg['volume'], name="Volume", marker_color='rgba(100, 150, 250, 0.5)'), row=2, col=1)

    #                 fig.update_layout(title=f"System Diagnostics: {dbg_symbol}", height=750, xaxis_rangeslider_visible=False, template="plotly_dark")
    #                 st.plotly_chart(fig, use_container_width=True)

    #                 # Raw Data Matrix Inspection
    #                 st.subheader("Gen-13 Matrix Output (Last 10 Days)")
    #                 st.dataframe(df_dbg.tail(10))
    #             else:
    #                 st.error(f"No valid data returned for {dbg_symbol}.")
        ...
        st.header("Visual Strategy Debugger (Gen-13 Log Replay)")
        st.markdown("Upload a system log to reconstruct AI decisions. The chart will automatically 'time-travel' to the exact date the log was generated.")

        uploaded_log = st.file_uploader("Upload System Log File (Live or Scanner .txt)", type=["txt", "log"])
        ...
        if uploaded_log is not None:
            ...
            for line in log_content.splitlines():
                # 1. Extract Time Machine bounds
                date_match = re.search(r"^(\d{4}-\d{2}-\d{2})\s(\d{2}:\d{2}:\d{2})", line)
                if date_match:
                    log_dates.append(f"{date_match.group(1)} {date_match.group(2)}")

                # 2. Parse Live Logs (with detailed SETUPS for UI Tools)
                if "SETUPS:" in line and "[StrategyEngine.analyze]" in line:
                    match = re.search(r"(\d{4}-\d{2}-\d{2}).*?\[([A-Z\.]+)\] SETUPS: \[(.*?)\] \| Tech: (.*?) \| AI: (.*?) \| Master: (.*)", line)
                    if match:
                        date_str, ticker, setups_raw, tech, ai, master = match.groups()
                        setups = [s.strip().strip("'").strip('"') for s in setups_raw.split(",") if s.strip() and s.strip() != "None"]
                        parsed_logs[ticker] = {
                            "date": date_str, "setups": setups, "tech_score": tech, "ai_score": ai, "master_score": master
                        }
                
                # 3. Parse Nightly Scanner Logs (Scores only)
                elif "Scan complete for" in line:
                    match = re.search(r"(\d{4}-\d{2}-\d{2}).*?Scan complete for ([A-Z\.]+): Master=([0-9\.]+), Tech=([0-9\.]+), AI=([0-9\.]+)", line)
                    if match:
                        date_str, ticker, master, tech, ai = match.groups()
                        if ticker not in parsed_logs:
                            parsed_logs[ticker] = {
                                "date": date_str, "setups": [], "tech_score": tech, "ai_score": ai, "master_score": master
                            }

            if not parsed_logs:
                st.info("Log file parsed, but no valid scan results found. Ensure it's a StockWise scanner or live log.")
            else:
                # Calculate automatic simulation bounds safely (extracting just the YYYY-MM-DD string)
                start_time = log_dates if log_dates else "Unknown"
                end_time = log_dates[-1] if log_dates else "Unknown"
                log_end_date = end_time.split(" ") if end_time != "Unknown" else None

                st.success(f"Log loaded successfully! Timeline reconstructed: `{start_time}` to `{end_time}`.")
                
                # Exclusively use tickers present in the log
                col_db1, col_db2, col_db3 = st.columns(3)
                ticker_list = sorted(list(parsed_logs.keys()))
                dbg_symbol = col_db1.selectbox("Target Ticker (From Log Only)", ticker_list, key="dbg_sym")
                dbg_days = col_db2.number_input("Historical Days Window", min_value=50, max_value=500, value=150)
                dbg_run = col_db3.button("Reconstruct Diagnostic Chart", use_container_width=True)

                log_entry = parsed_logs[dbg_symbol]
                setups_found = log_entry["setups"]

                # Dynamic Tool Checkboxes based on logged setups
                used_rsi = any("OVERSOLD" in s or "RSI" in s for s in setups_found)
                used_macd = any("MOMENTUM" in s or "MACD" in s or "TREND" in s for s in setups_found)
                used_bb = any("VOLATILITY" in s or "MOMENTUM" in s for s in setups_found)
                used_kc = any("VSA" in s or "INSTITUTIONAL" in s for s in setups_found)

                st.sidebar.subheader("System Tools Used (Log Data)")
                show_sma = st.sidebar.checkbox("Show SMA (20, 50)", value=True)
                show_bb = st.sidebar.checkbox("Show Bollinger Bands", value=used_bb)
                show_kc = st.sidebar.checkbox("Show Keltner Channels", value=used_kc)
                show_atr_stop = st.sidebar.checkbox("Gen-13 Dynamic Stop (ATR)", value=True)
                show_rsi = st.sidebar.checkbox("Show RSI Momentum", value=used_rsi)
                show_macd = st.sidebar.checkbox("Show MACD Trend", value=used_macd)

                # AI Metadata Panel
                st.subheader(f"AI Matrix Diagnostics for {dbg_symbol}")
                st.markdown(f"**Log Generated:** {log_entry['date']} | **Tech Score:** {float(log_entry['tech_score']):.2f} | **AI Score:** {float(log_entry['ai_score']):.2f} | **Master Score:** {float(log_entry['master_score']):.2f}")
                
                if setups_found:
                    st.markdown(f"**Tools & Setups Triggered:** `{', '.join(setups_found)}`")
                else:
                    st.markdown("**Tools & Setups Triggered:** `Advanced AI Matrix Pattern (No manual UI setup logged)`")

                if dbg_run and dbg_symbol:
                    with st.spinner(f"Reconstructing historical matrix for {dbg_symbol} exactly as seen on {log_end_date}..."):
                        
                        # Fix Alpaca ticker formatting to prevent API Data Starvation (e.g. BRK.B -> BRK-B)
                        alpaca_symbol = dbg_symbol.replace(".", "-")
                        df_dbg = st.session_state.data_manager.get_stock_data(alpaca_symbol, days_back=dbg_days)
                        
                        if df_dbg is not None and not df_dbg.empty:
                            # Time Machine: Truncate dataframe to log date securely
                            if log_end_date:
                                try:
                                    # Ensure the index is time-zone naive for clean slicing
                                    if df_dbg.index.tz is not None:
                                        df_dbg.index = df_dbg.index.tz_localize(None)
                                        
                                    target_date = pd.to_datetime(log_end_date)
                                    df_dbg = df_dbg[df_dbg.index <= target_date].copy()
                                except Exception as e:
                                    st.warning(f"Time slicing adjustment warning: {e}")
                                    
                            if df_dbg.empty:
                                st.error(f"Simulation Failed: Data Starvation. After slicing to log date {log_end_date}, no data remains for {dbg_symbol}.")
                            else:
                                calc = st.session_state.advisor.calculator
                                try:
                                    df_dbg = calc.calculate_features(df_dbg, context_data=st.session_state.contextual_data)
                                ...
                                # Find dynamic columns from pandas_ta
                                rsi_col = next((c for c in df_dbg.columns if c.startswith('RSI')), None)
                                macd_col = next((c for c in df_dbg.columns if c.startswith('MACD_')), None)
                                macds_col = next((c for c in df_dbg.columns if c.startswith('MACDs_')), None)
                                macdh_col = next((c for c in df_dbg.columns if c.startswith('MACDh_')), None)
                                kcu_col = next((c for c in df_dbg.columns if c.startswith('KCU')), None)
                                kcl_col = next((c for c in df_dbg.columns if c.startswith('KCL')), None)

                                # Build Interactive Chart (Supports Native Zoom)
                                ...
                                if show_kc and kcu_col and kcl_col:
                                    fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg[kcu_col], line=dict(color='rgba(0,255,0,0.5)', width=1, dash='dash'), name="KC Upper"), row=1, col=1)
                                    fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg[kcl_col], line=dict(color='rgba(0,255,0,0.5)', width=1, dash='dash'), name="KC Lower"), row=1, col=1)
                                ...
                                if show_rsi and rsi_col:
                                    fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg[rsi_col], line=dict(color='yellow', width=1.5), name="RSI"), row=3, col=1)
                                    fig.add_hline(y=70, line_dash="dot", line_color="red", row=3, col=1)
                                    fig.add_hline(y=30, line_dash="dot", line_color="green", row=3, col=1)

                                if show_macd and macd_col and macds_col:
                                    fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg[macd_col], line=dict(color='white', width=1.5), name="MACD"), row=4, col=1)
                                    fig.add_trace(go.Scatter(x=df_dbg.index, y=df_dbg[macds_col], line=dict(color='red', width=1), name="Signal"), row=4, col=1)
                                    if macdh_col:
                                        colors = ['rgba(0, 255, 0, 0.5)' if val >= 0 else 'rgba(255, 0, 0, 0.5)' for val in df_dbg[macdh_col]]
                                        fig.add_trace(go.Bar(x=df_dbg.index, y=df_dbg[macdh_col], name="Histogram", marker_color=colors), row=4, col=1)

                                fig.update_layout(
                                    title=f"Reconstructed Diagnostics: {dbg_symbol} | End Date: {log_end_date}", 
                                    height=950, 
                                ...
                                st.subheader(f"Gen-13 Matrix Output (Reconstructed timeline for {log_end_date})")
                                st.dataframe(df_dbg.tail(10))
                        else:
                            st.error(f"Simulation Failed: Data Starvation: ALPACA API returned no data for {alpaca_symbol}.")
        else:
    ...
    # -------------------------------------------------------------------------
    if mico_optimizer is None:
        st.error("Optimizer module is not installed in Gen-12 environment.")
    else:
            ...
            if not stock_universe:
                ...
                if not active_advisors:
                    ...
                    with st.spinner(f"Running parallel scan for {len(stock_universe)} symbols..."):
                        ...
                        recommended_trades_list = stock_hunter.run_full_market_scan(
            ...
            if not active_advisors:
                ...
                if not stock_universe:
                    ...
                    for i, day in enumerate(trading_days):
                        ...
                        daily_trades_list = stock_hunter.run_full_market_scan(
                            universe_tickers=stock_universe,
...
if __name__ == "__main__":
    ...
    try:
        if authentication_status:
            ...
            # --- SAFE INITIALIZATION OF LEGACY ADVISORS ---
            # These classes are missing in Gen-12. We set them to None to prevent NameError crashes.
            if 'mean_reversion_advisor' not in st.session_state:
                st.session_state.mean_reversion_advisor = None 

            if 'breakout_advisor' not in st.session_state:
                st.session_state.breakout_advisor = None

            if 'supertrend_advisor' not in st.session_state:
                st.session_state.supertrend_advisor = None

            if 'ma_crossover_advisor' not in st.session_state:
                st.session_state.ma_crossover_advisor = None

            if 'volume_momentum_advisor' not in st.session_state:
                st.session_state.volume_momentum_advisor = None
...
# """
# StockWise Gen-10: Interactive Command Center (Streamlit)
# ========================================================
# A web-based dashboard to:
# 1. Visualize Gen-10 Strategy on any stock (Simulation Mode).
# 2. Monitor the Live/Unified Engine performance (Live Monitor).

# Usage:
#     streamlit run stockwise_simulation.py
# """
...
# # --- PAGE CONFIG ---
# st.set_page_config(page_title="StockWise Gen-10", layout="wide", page_icon="🎯")

# # --- SIDEBAR ---
# st.sidebar.title("🎯 StockWise Gen-10")
# mode = st.sidebar.radio("Select Mode", ["🔬 Strategy Lab", "📺 Live Monitor"])

# # --- MODE 1: STRATEGY LAB (Simulation) ---
# if mode == "🔬 Strategy Lab":
#     st.title("🔬 Gen-10 Strategy Laboratory")
#     st.markdown("Test the AI & Strategy Logic on any stock instantly.")

#     # 1. Inputs
#     col1, col2, col3 = st.columns(3)
#     with col1:
#         symbol = st.selectbox("Select Ticker", cfg.TRAINING_SYMBOLS, index=0)
#     with col2:
#         days_back = st.number_input("Days Back", min_value=100, max_value=2000, value=400)
#     with col3:
#         run_btn = st.button("🚀 Run Simulation", type="primary")

#     if run_btn:
#         status_text = st.empty()
#         status_text.info(f"Fetching Data for {symbol}...")
...
#         qqq_df = dsm.get_stock_data("QQQ", days_back=days_back+100, interval='1d')
#         context = {'qqq': qqq_df} if qqq_df is not None else {}
...
#         status_text.info("Calculating Gen-10 Features...")
#         df = calc.calculate_features(df, context_data=context)
...
#                 st.warning(f"⚠️ No custom model found for {symbol}. Using generic/fallback if available.")

#         # D. Run Loop (Fast Version)
#         status_text.info("Simulating Trades...")
...
#             # Update progress every 10 days to be fast
#             if i % 10 == 0: progress_bar.progress((i - 60) / total_steps)
...
#             # Mock fundamentals for speed in playground
#             funds = {'trailingPE': 20, 'revenueGrowth': 0.2} 
...
#             analysis = {'AI_Probability': prob, 'Fundamental_Score': fund_score}
#             verdict = StrategyOrchestra.decide_action(symbol, today, analysis)
            
#             price = today['close']
            
#             # 3. Trade Logic (With Trailing Stop)
#             if active_trade:
#                 # Trailing Stop Update
#                 if True: # Always enabled in lab
#                     profit_pct = (price - active_trade['entry']) / active_trade['entry']
#                     if profit_pct > 0.05:
#                         atr = today.get('atr', price*0.02)
#                         new_stop = price - (atr * 2.0)
#                         if new_stop > active_trade['stop']:
#                             active_trade['stop'] = new_stop
                
#                 # Check Exit
#                 reason = None
#                 if today['low'] <= active_trade['stop']: reason = "Stop Loss"
#                 elif today['high'] >= active_trade['target']: reason = "Target"
                
#                 if reason:
#                     exit_price = active_trade['stop'] if reason == "Stop Loss" else active_trade['target']
#                     pnl = (exit_price - active_trade['entry']) / active_trade['entry']
#                     trades.append({
#                         'Entry Date': active_trade['date'], 'Exit Date': idx,
#                         'Entry': active_trade['entry'], 'Exit': exit_price, 'PnL': pnl, 'Reason': reason
#                     })
...
#                 active_trade = {
#                     'entry': price, 'stop': stop, 'target': target, 'date': idx
#                 }
...
#         # 1. Get Investment Amount from Config
#         # Fallback to 10,000 if not found in cfg
#         investment_amount = getattr(cfg, 'INVESTMENT_AMOUNT', 1000.0)
        
#         # 2. Calculate Metrics
#         total_entries = len(trades)
#         wins = [t for t in trades if t['PnL'] > 0]
#         win_count = len(wins)
#         loss_count = total_entries - len(wins)
        
#         win_rate = (win_count / total_entries * 100) if total_entries > 0 else 0
#         loss_rate = 100.0 - win_rate
        
#         # Unleveraged Return % (Sum of PnL percentages)
#         total_pnl_pct = sum([t['PnL'] for t in trades]) 
        
#         # Profit in Dollars based on Investment Amount
#         profit_dollars = total_pnl_pct * investment_amount
        
#         # 3. Display Metrics
#         m1, m2, m3 = st.columns(3)
        
#         # Shows Total Entries AND Wins
#         m1.metric("Total Entries", f"{total_entries} (Wins: {win_count})")
        
#         # Shows Win AND Loss Rate
#         m2.metric("Win / Loss Rate", f"{win_rate:.1f}% / {loss_rate:.1f}%")
        
#         # Shows Investment and Profit Amount
#         m3.metric(
#             label=f"Profit (Inv: ${investment_amount:,.0f})", 
#             value=f"${profit_dollars:,.2f}", 
...
#         fig.add_trace(go.Scatter(x=buys.index, y=buys['buy_signal'], mode='markers',
#                                  marker=dict(symbol='triangle-up', color='#00FF00', size=15), name='BUY'), row=1, col=1)
#         fig.add_trace(go.Scatter(x=sells.index, y=sells['sell_signal'], mode='markers',
#                                  marker=dict(symbol='triangle-down', color='#FF0000', size=15), name='SELL'), row=1, col=1)
        
#         # AI
#         fig.add_trace(go.Scatter(x=df.index, y=df['ai_conf'], fill='tozeroy', 
#                                  line=dict(color='purple'), name='AI Confidence'), row=2, col=1)
...
#         # 3. Trade Table
#         if trades:
#             st.dataframe(pd.DataFrame(trades).style.format({
#                 'Entry': '{:.2f}', 'Exit': '{:.2f}', 'PnL': '{:.2%}'
#             }))

# # --- MODE 2: LIVE MONITOR ---
# elif mode == "📺 Live Monitor":
#     st.title("📺 Live Portfolio Monitor")
#     st.markdown("Tracking the `unified_trading_engine.py` performance.")
    
#     trade_file = os.path.join("debug_charts", "portfolio_trades.csv")
...
#                 c2.metric("Win / Loss Rate", f"{wr:.1f}% / {lr:.1f}%")
#                 c3.metric("Total Trades", len(df_trades))
                
#                 st.subheader("Recent Trades")
#                 st.dataframe(df_trades.tail(10).sort_index(ascending=False))
                
#                 # Simple Equity Curve (Simulated)
#                 st.subheader("Performance Curve")
#                 df_trades['Cumulative PnL'] = df_trades['PnL_USD'].cumsum()
#                 st.line_chart(df_trades['Cumulative PnL'])
#             else:
#                 st.info("No closed trades yet.")
#         except Exception as e:
#             st.error(f"Error reading trade log: {e}")
#     else:
#         st.warning("No trade history found. Run the Unified Engine first!")
